...
describe Ledger do
  ...
  describe '#settle!' do
    ...
    it 'chooses one payment that cancels two accounts first' do
      expect(subject.payments[0]).to eq(Transaction.new(:I, :H, 7))
    end

    it 'chooses two payments that cancel three accounts second' do
      expect(subject.payments[1]).to eq(Transaction.new(:G, :E, 25))
      expect(subject.payments[2]).to eq(Transaction.new(:G, :F, 50))
    end

    it 'chooses one payment that cancels one account next' do 
      expect(subject.payments[3]).to eq(Transaction.new(:B, :A, 2))
    end
